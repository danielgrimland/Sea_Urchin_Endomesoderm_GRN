...
def generate_constraint_appearences():
    ...
    for experiment_n in CONSTRAINT_EXPRESSIONS:
        ...
        for constraint_n in CONSTRAINT_EXPRESSIONS[experiment_n]:
            ...
            new_expressions[experiment_n].append(Constraint_Appearences(f"Restriction_{experiment_n}_{constraint_n}", appearences, constraint_n))
        ...
        if (not(ORDER)):
            for appearences in expressions[experiment_n]:
                for _ in range(len(appearences.appearences)):
                    corresponding_constraint_names.append(appearences.name)
...
def format_order(experiments, order_elements):
    for e_number in ORDER:
        if (e_number not in experiments):
            f.error(f"The ORDER provided assign a variable (possibly not integer) an order, despite not being defined in the CONSTRAINT EXPRESSIONS as experiment.")
...
if (len(PERTURBATIONS) != 0):
    format_perturbations(constraints)

    perturbations = generate_perturbations(translate_perturbations(components))
