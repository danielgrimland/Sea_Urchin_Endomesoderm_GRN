...
PERTURBATIONS = [["COMB", [f.tuplize(get_components()), 1, [1], 1]]]
...
def format_constraint_expressions(constraints, experiment_numbers):
    experiments = []
    ...
    for e_number in CONSTRAINT_EXPRESSIONS:
        if (e_number not in experiment_numbers):
            f.error(f"The experiment number {e_number} was not specified in the untranslated data provided.")
...
for e_number in constraints:
    e_numbers.append(e_number)

if (len(CONSTRAINT_EXPRESSIONS) != 0):
    experiment_numbers, order_elements = format_constraint_expressions(constraints, e_numbers)
