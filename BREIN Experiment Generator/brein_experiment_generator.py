    ...
    for e_number in ORDER:
        if (e_number not in experiment_numbers):
            ...
            for i in range(len(ORDER[e_number])):
                ...
                elif(str(ORDER[e_number][i][0]) not in constraints[e_number]):
                    f.error(f"Constraint {e_number[i][0]} of constarint number {i + 1} of experiment number {e_number} is not in the untranslated data given.")
