...
The first place specifies the name of the component perturbed.
The second place specifies in which experiments to perturb the component.
The third place specifies whether to overexpress or knockout the component (1 and 0 respectively).
Use the ALL keyword as the name to specify all components should be, and in different files.
Use it as the experiment number to specify all it should be perturbed in all experiments
...
def Add_Constraints_To_Timestamps(timestamp_list, experiment_list, COMPONENT_LIST, KIND, perturbed_list):
    ...
    if (KIND == "Range"):
        for c in sliced_constraint_data:
            if c[3][0][0] == "P":
                timestamp_list[c[1] - 1][0].append([c[2], c[3][0][1]])
        ...
        for c in sliced_constraint_data:
            if c[3][0] == "P":
                timestamp_list[c[1] - 1][0].append([c[2], c[3][1]])
        ...
        Error("ERROR: Illegal or missing KIND (OSTRING or Range)")
...
def Create_Experiments(timestamp_list, final_string_list, final_name_lists, count):
    for i in range(NUM_OF_EXPERIMENTS):
        file_name = f"{OUTPUT_NAME}_{final_name_lists[i]}_fz-{FUZZY_NUMBER}_st-{STRANDING_NUMBER}_{i}_{count}.txt"

        if (KEEP_FRAC != 1):
            file_name = f"{OUTPUT_NAME}_{final_name_lists[i]}_fz-{FUZZY_NUMBER}_st-{STRANDING_NUMBER}_frac-{KEEP_FRAC}_{i}_{count}.txt"
...
def Run_With_Perturbations():
    if FE_KO_LIST[0][0] == "ALL":
        ...
        for c in COMPONENT_LIST:
            ...
            for exp in FE_KO_LIST[0][1]:

                if KIND == "OString":
                    if (FE_KO_LIST[0][2] == 0):
                        line_list.append([f"#{exp}|KO({c})|P{FE_KO_LIST[0][2]}", -1])
                    else:
                        line_list.append([f"#{exp}|FE({c})|P{FE_KO_LIST[0][2]}", -1])
            ...
            Run(line_list, perturbed_list, count)
...
def Run(line_list, perturbed_list, count):
    ...
    timestamp_list = Add_Constraints_To_Timestamps(timestamp_list, experiment_list, COMPONENT_LIST, KIND, perturbed_list)
    ...
    Create_Experiments(timestamp_list, final_string_list, final_name_lists, count)
