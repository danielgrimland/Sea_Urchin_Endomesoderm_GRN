...
class State_Mapping:
    ...
    def monotonic(self):
        ...
        if ((self.dict.get((min(self.domain_values), min(self.domain_values))) != self.dict.get((min(self.domain_values[1:]), min(self.domain_values[1:])))) or 
            (self.dict.get((max(self.domain_values[:-1]), min(self.domain_values))) != self.dict.get((max(self.domain_values[:-1]), min(self.domain_values[1:])))) or 
            (self.dict.get((min(self.domain_values), max(self.domain_values[:-1]))) != self.dict.get((min(self.domain_values[1:]), max(self.domain_values[:-1]))))):
            return False
