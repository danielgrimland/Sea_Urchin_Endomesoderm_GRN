    ...
    for interaction in interactions:
        if ((interaction.delays[0] != 1) or (len(interaction.delays) != 1)):
            ...
            for i in range(2, max_delay + 1):
                new_components.append(Component(f"{interaction.initiator}-{interaction.target}-{i}", 1, 17))
            
            new_interactions.append(Interaction(interaction.initiator, f"{interaction.initiator}-{interaction.target}-2", "pos", "def", 1))
            
            for i in range(2, max_delay):
                new_interactions.append(Interaction(f"{interaction.initiator}-{interaction.target}-{i}", f"{interaction.initiator}-{interaction.target}-{i + 1}", "pos", "def", 1))
                
                if (i in interaction.delays):
                    new_interactions.append(Interaction(f"{interaction.initiator}-{interaction.target}-{i}", interaction.target, interaction.is_pos, interaction.is_def, 1))

            new_interactions.append(Interaction(f"{interaction.initiator}-{interaction.target}-{max_delay}", interaction.target, interaction.is_pos, interaction.is_def, 1))
